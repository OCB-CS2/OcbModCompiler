...
namespace MODCS2
{
    class APCS2
    {
        ...
        {
            Assembly assemby = null;
            var parts = args.Name.Split(",");
            Console.WriteLine("  resolving: " + parts[0] + ".dll");
            var fqdn = ManagedPath + "\\" + parts[0] + ".dll";
            if (File.Exists(fqdn))
            {
                assemby = Assembly.LoadFile(fqdn);
                Console.WriteLine("  resolved: " + assemby);
            }
            fqdn = BepInExPath + "\\" + parts[0] + ".dll";
            if (File.Exists(fqdn))
            {
                assemby = Assembly.LoadFile(fqdn);
                Console.WriteLine("  resolved: " + assemby);
        ...
        */
        ...
        static int Main(string[] args)
        {
            ...
            // Seems not really required, as resolver works
            ...
            BepInExPath = ManagedPath + "\\..\\..\\BepInEx\\core";
            ...
            resolver.AddSearchDirectory(BepInExPath);
            string assemblyPath = ManagedPath + @"\Game.dll";
